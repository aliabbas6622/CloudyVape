...
import { useGetAdminMe, useAdminLogout, getGetAdminMeQueryKey, useListCategories, useGetSettings } from "@/api/hooks";
import { Button } from "@/components/ui/button";
import siteLogo from "../../../image.png";
...
export function Navbar() {
  ...
  return (
    <>
      {/* Main navbar */}
      <header className={cn(
        "sticky top-0 z-40 w-full transition-all duration-300 backdrop-blur-xl border border-border/70 border-t-0 rounded-b-[32px] overflow-hidden",
        scrolled
        ...
        <div className="container mx-auto px-4 h-18 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="flex h-11 w-11 items-center justify-center rounded-3xl bg-white shadow-md overflow-hidden">
              <img src={siteLogo} alt="Cloudy Vapours" className="h-full w-full object-cover" />
            </div>
          ...
          <nav className="hidden md:flex items-center gap-7">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="bg-popover border-border min-w-[200px] mt-2 rounded-xl shadow-lg p-1.5">
                {categories?.map((cat) => (
                  <DropdownMenuItem
                    key={cat.id}
          ...
          <div className="hidden md:flex items-center gap-3">
            {isSearchOpen ? (
              <form onSubmit={handleSearch} className="relative flex items-center animate-in fade-in slide-in-from-right-4 duration-200">
                <input
                  type="text"
            ...
            <Button
              className="rounded-full bg-[#25D366] hover:bg-[#128C7E] text-white font-semibold h-9 px-4 shadow-sm"
        ...
        {/* Mobile menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-border bg-background px-4 py-5 space-y-5 shadow-lg animate-in slide-in-from-top-2">
