...
export default function ProductDetail() {
    ...
    return (
      <div className="container mx-auto px-4 py-12 md:py-20 animate-in fade-in duration-500">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20">
          <Skeleton className="aspect-square w-full rounded-3xl bg-muted border border-border" />
          <div className="space-y-6 pt-4">
            <Skeleton className="h-6 w-1/3 bg-muted" />
            <Skeleton className="h-12 w-3/4 bg-muted" />
            <Skeleton className="h-10 w-1/4 bg-muted" />
            <Skeleton className="h-4 w-full bg-muted" />
            <Skeleton className="h-4 w-full bg-muted" />
            <Skeleton className="h-4 w-2/3 bg-muted" />
            <Skeleton className="h-14 w-full rounded-full bg-muted mt-8" />
          </div>
        </div>
      </div>
    );
  ...
  if (isError || !product) {
    return (
      <div className="container mx-auto px-4 py-32 text-center min-h-[60vh] flex flex-col items-center justify-center">
        <Package className="h-24 w-24 text-foreground/10 mb-6" />
        <h2 className="text-4xl font-display font-bold text-foreground mb-4">Product Not Found</h2>
        <p className="text-muted-foreground mb-8 text-lg">The item you're looking for doesn't exist or has been removed.</p>
        <Button onClick={() => navigate("/products")} className="rounded-full bg-primary hover:bg-primary/90 text-white px-8 h-12">
          Browse All Products
        </Button>
      </div>
    );
  ...
  const productImages = product.imageUrls && product.imageUrls.length > 0
    ? product.imageUrls
    : [getFallbackImage(product.categoryName)];
        ...
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-start">
                ...
                {productImages.map((img, index) => (
                  <button
                    ...
                    className={"overflow-hidden rounded-2xl border transition-all " +
                      (selectedImageIndex === index ? "border-primary shadow-lg" : "border-border bg-muted")}
                  >
          ...
          <div className="flex flex-col space-y-8">
            ...
            <div className="grid grid-cols-3 gap-4 pt-6">
              <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-muted/40 border border-border text-center gap-2">
                <ShieldCheck className="h-6 w-6 text-primary" />
                <span className="text-xs font-semibold text-foreground/80">Authentic Product</span>
              </div>
              <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-muted/40 border border-border text-center gap-2">
                <MessageCircle className="h-6 w-6 text-primary" />
                <span className="text-xs font-semibold text-foreground/80">WhatsApp Support</span>
              </div>
              <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-muted/40 border border-border text-center gap-2">
                <Clock className="h-6 w-6 text-primary" />
                <span className="text-xs font-semibold text-foreground/80">Always Available</span>
              </div>
            </div>
